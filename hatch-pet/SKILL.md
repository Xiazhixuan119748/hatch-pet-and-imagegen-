...
Keep image generation isolated and bounded:
...
- Use `$imagegen` as the only visual generation layer. Approve `$imagegen`'s bundled `scripts/image_gen_with_codex_env.py` wrapper for every visual job; do not invoke image APIs, other image CLIs, local raster generators, or one-off generation scripts from this skill.
