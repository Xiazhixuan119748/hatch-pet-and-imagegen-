...
- OpenAI 模式不再强制要求 `OPENAI_BASE_URL`；未配置时使用官方 `https://api.openai.com/v1`，仍可按需填写兼容中转站地址。
- OpenAI、Gemini、Grok 和 Agnes 命令新增显式 `--aspect-ratio W:H` 校验，要求 `--size` 与目标宽高比精确匹配，并在交付前验证实际图片尺寸。
- OpenAI CLI 为输出文件增加生成锁和冲突保护，避免长请求超时后被重复提交或覆盖同一路径。
- OpenAI 批任务改为单次请求、无自动重试，并记录每个任务的状态与汇总；部分成功时会保留并交付已验证的图片。
- 发布范围仅保留两个 Skill 的运行文件、参考资料、许可证、安装文档和脱敏配置模板，不包含测试目录、Python 缓存或本地代理配置。
...
- `IMAGE_PROVIDER=openai` 时只要求 `OPENAI_API_KEY`；未配置 `OPENAI_BASE_URL` 时使用官方 `https://api.openai.com/v1`。
- 配置 `OPENAI_BASE_URL` 时应填写 OpenAI SDK 兼容基础地址，通常以 `/v1` 结尾。
...
OpenAI 模式检查 `OPENAI_API_KEY`；未填写 `OPENAI_BASE_URL` 时使用官方地址。Gemini 模式检查 `GEMINI_API_KEY`；Grok 模式检查 `XAI_API_KEY`；Agnes 模式检查 `AGNES_API_KEY`。同时检查 `IMAGE_PROVIDER` 拼写、等号右侧内容以及文件路径。
